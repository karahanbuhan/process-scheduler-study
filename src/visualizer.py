...
def visualize_results(schedule, waiting_times, avg_waiting_time, mode='gui', algorithm='Unknown', processes=None):
    """
    ...
    if mode == 'gui':
        ...
        # Süreçlerin burst time'larını başlık için hazırla
        burst_times_str = 'No Processes'
        if processes:
            try:
                burst_times_str = ', '.join([f"{p['pid']}={p['burst_time']}" for p in processes])
        ...
        # Başlığa algoritma ve burst time'ları ekle
        ax.set_title(f'Gantt Chart ({algorithm}, Processes: {burst_times_str}, Avg Waiting Time: {avg_waiting_time:.2f} = {n}/{d})')
        ax.set_xlabel('Time')
